...
int main()
{
	...
	*pWert = --Wert + 8; // pWert is a pointer, to save to its reference: *pWert
	...
	// Ausgaben 2
	printf("%i, %i\n", Zahl, *pZahl); // logik fehler: POinter *pZahl, von Zeile 23; zeigt undefinierten Speicherplatz
	printf("%c, %c\n", Zeichen, *pZeichen);
	printf("%lf, %lf\n", *(pWert + 5), Wert); // logik fehler: Pointer pWert +5 zeigt auf undefinierten Speicherplatz
